...
test_that("mangle conflicts with mangle_key a new storr", {
  on.exit(options(storr_mangler = NULL))
  do.call(register_mangler, test_mangler)
  expect_error(
    storr_rds(tempfile(), mangle_key = "test_mangler_2"),
    regex = "conflicts with mangle_key"
  )
})

test_that("overwriting registered manglers", {
  on.exit(options(storr_mangler = NULL))
  do.call(register_mangler, test_mangler)
  test_mangler_2 <- test_mangler
  test_mangler_2$name <- "test_mangler_2"
  do.call(register_mangler, test_mangler_2)
  s <- storr_rds(tempfile(), mangle_key = "test_mangler")
  test_key(s, "a", "test_mangler_a")
  test_mangler_2$overwrite = TRUE
  expect_warning(
    do.call(register_mangler, test_mangler_2),
    regex = "Overwriting existing key mangler"
  )
  expect_error(
    storr_rds(tempfile(), mangle_key = "test_mangler"),
    regex = "conflicts with mangle_key"
  )
  s <- storr_rds(tempfile(), mangle_key = "test_mangler_2")
  test_key(s, "a", "test_mangler_a")
})

test_that("new mangler global option conflicts with old stored mangler", {
  on.exit(options(storr_mangler = NULL))
  do.call(register_mangler, test_mangler)
  s <- storr_rds(tempfile(), mangle_key = "test_mangler")
  do.call(
...
test_that("new custom mangler conflicts with old stored one", {
  on.exit(options(storr_mangler = NULL))
