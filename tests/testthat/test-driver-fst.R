context("driver fst details")

## Tests of the implementation details of the fst driver only...
test_that("creation", {
  path <- tempfile()
...
test_that("mangling", {
  path <- tempfile()
  dr <- driver_fst(path, mangle_key = TRUE)
  on.exit(dr$destroy())
  st <- storr(dr)
...
test_that("mangledless compatibility", {
  ...
  dr1 <- driver_fst(path, mangle_key = TRUE)
  ...
  expect_error(driver_fst(path, mangle_key = FALSE),
  ...
  ## Pointing another driver here without mangling is an error:
  expect_error(driver_fst(path2, mangle_key = TRUE),
               "Incompatible value for mangle_key")

  ## But omitting the argument (NULL mangledness) is OK
  dr4 <- driver_fst(path2)
...
test_that("mangledness padding compatibility", {
  ...
  ## Pointing another driver here without mangling is an error:
  expect_error(driver_fst(path, mangle_key_pad = TRUE),
  ...
  dr3 <- driver_fst(path2, mangle_key = TRUE, mangle_key_pad = TRUE)
  ...
  ## Pointing another driver here without mangling is an error:
  expect_error(driver_fst(path2, mangle_key = TRUE, mangle_key_pad = FALSE),
               "Incompatible value for mangle_key")
...
test_that("large vector support", {
  ...
  path <- tempfile()
  dr <- driver_fst(path, compress = FALSE)
  on.exit(dr$destroy())
  st <- storr(dr)
...
test_that("compression support", {
  ...
  st1 <- storr_fst(tempfile(), TRUE)
  st2 <- storr_fst(tempfile(), FALSE)
  on.exit({
    st1$destroy()
    st2$destroy()
  })
...
test_that("backward compatibility", {
  ...
  path <- copy_to_tmp("v1.0.1_clear")
  expect_error(storr_fst(path, hash_algorithm = "sha1"),
...
test_that("vectorised exists", {
  st <- storr_fst(tempfile(), mangle_key = TRUE)
  on.exit(st$destroy())
...
test_that("recover corrupt storr: corrupted fst", {
  st <- storr_fst(tempfile())
...
test_that("automatic is ok if storr is healthy", {
  expect_false(storr_fst(tempfile())$repair(force = FALSE))
})


test_that("corrupted mangled keys", {
  st <- storr_fst(tempfile(), mangle_key = TRUE, mangle_key_pad = TRUE)
  st$mset(month.name,
...
test_that("avoid race condition when writing in parallel", {
  ...
  ##
  ## Try and hammer the fst storr with concurrent writes.  The end
